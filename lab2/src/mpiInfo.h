 ...
 public:
  ...
  void ExchangeBoundaryInfo(VD &Solution, VD &b)
  {
	...
	// (1.2) Send them to neighboring PEs
        ...
        if ( nei_n >= 0 )  err = MPI_Isend( /* TO-DO in Lab */  , &request );
	if ( nei_s >= 0 )  err = MPI_Isend( /* TO-DO in Lab */  , &request );
	if ( nei_e >= 0 )  err = MPI_Isend( /* TO-DO in Lab */  , &request );
	if ( nei_w >= 0 )  err = MPI_Isend( /* TO-DO in Lab */  , &request );

	// (1.3) Receive values from neighobring PEs' physical boundaries.
	
        if ( nei_n >= 0 ) { err = MPI_Irecv( /* TO-DO in Lab */ , &request );   MPI_Wait(&request,&status); } 
	if ( nei_s >= 0 ) { err = MPI_Irecv( /* TO-DO in Lab */ , &request );   MPI_Wait(&request,&status); }
	if ( nei_e >= 0 ) { err = MPI_Irecv( /* TO-DO in Lab */ , &request );   MPI_Wait(&request,&status); }
	if ( nei_w >= 0 ) { err = MPI_Irecv( /* TO-DO in Lab */ , &request );   MPI_Wait(&request,&status); }
