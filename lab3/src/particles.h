...
class particles {
 public:
  ...
  double
      Qp;  // Extensive physical quantity represented by each particle (C/ptcl)
  ...
  double
      nHat;  // Number of particles/area to be injected, when injection occurs.
  ...
  void addFlux(double time, double dt, double y0, double y1, int myPE) {
    if (injectionInterval < dt) {
      ...
      FatalError(
          "injectionInterval is too small.  Increase the particle injection "
          "density or decrease dt.");
    }
    ...
    // So we are late!  Calculate how far into the mesh the particles have
    // traveled since entering.
